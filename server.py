...
_MANAGED_KEYS = {
    "RED_BRAIN_TYPE", "RED_API_KEY", "RED_BASE_URL", "RED_MODEL",
    "BLUE_BRAIN_TYPE", "BLUE_API_KEY", "BLUE_BASE_URL", "BLUE_MODEL",
    "LLM_INTERVAL", "TPS",
    "NVIDIA_API_KEY", "LLM_BASE_URL", "LLM_MODEL", "LLM_COLONY",
}
...
def _write_env():
    ...
    # Preserve any lines not managed by this function (e.g. AGANTS_AUTH_URL)
    preserved = []
    try:
...
class Server:
    ...
    async def api_delete_match(self, req):
        ...
        match_id = req.match_info["match_id"]
        if match_id == self._default_match_id:
            return await self._api_cors(web.json_response({"error": "cannot delete default match"}, status=400))
        m = self.matches.get(match_id)
        if m is None:
            return await self._api_cors(web.json_response({"error": "match not found"}, status=404))
        if m.world.phase != "lobby":
            return await self._api_cors(web.json_response({"error": "can only delete lobby matches"}, status=400))
        if any(v for v in m.world.mcp_seats.values()):
            return await self._api_cors(web.json_response({"error": "seat occupied — release first"}, status=409))
    ...
    async def api_heartbeat(self, req):
        """Mark an agent as online without needing a seat. Body: {api_key}."""
        try:
            body = await req.json()
        except Exception:
            body = {}
        ...
        if AGANTS_AUTH_URL:
            ...
            return await self._api_cors(web.json_response({"error": "api_key required"}, status=400))
        key = f"lobby:{user_id}"
        self._presence[key] = {
            "agent":     username,
            "user_id":   user_id,
        ...
        return await self._api_cors(web.json_response({"ok": True, "username": username}))
    ...
    async def api_agents_online(self, req):
        ...
        for entry in by_user.values():
            ...
            agents.append({
                ...
                "colony_id":  cid,
                "colony":     ("RED" if cid == 0 else "BLUE" if cid == 1 else None),
    ...
    EMPTY_LOBBY_TTL_M = int(os.environ.get("EMPTY_LOBBY_TTL_M", 30))

    async def _match_cleanup_loop(self):
        """Remove ended matches older than MATCH_TTL_H hours, and empty lobbies older than
        EMPTY_LOBBY_TTL_M minutes. Runs every 5 minutes."""
        while True:
            await asyncio.sleep(300)
            now = time.time()
            ended_cutoff = now - self.MATCH_TTL_H * 3600
            lobby_cutoff = now - self.EMPTY_LOBBY_TTL_M * 60
            stale = []
            for mid, m in list(self.matches.items()):
                if mid == self._default_match_id:
                    continue
                if m.ended_at is not None and m.ended_at < ended_cutoff:
                    stale.append(mid)
                elif (m.world.phase == "lobby"
                      and not any(v for v in m.world.mcp_seats.values())
                      and m.created_at < lobby_cutoff):
                    stale.append(mid)
            for mid in stale:
